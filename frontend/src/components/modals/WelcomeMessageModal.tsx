...
import { X, Plus, Trash2, Image, Video, MessageCircle, Clock, Link2, Bold, Italic, Underline, Strikethrough, Code, Link as LinkIcon } from 'lucide-react';
...
export const WelcomeMessageModal: React.FC<WelcomeMessageModalProps> = ({ chat, onClose }) => {
  ...
  const applyFormatting = (formatType: 'bold' | 'italic' | 'underline' | 'strikethrough' | 'code' | 'spoiler' | 'link') => {
    const textarea = textareaRef.current;
    if (!textarea) return;
    ...
    switch (formatType) {
      ...
      case 'code':
        ...
        break;
      case 'spoiler':
        formattedText = `<span class="tg-spoiler">${selectedText}</span>`;
        cursorOffset = 26;
        break;
      case 'link':
    ...
    setTimeout(() => {
      textarea.focus();
      const newCursorPosition = start + cursorOffset + selectedText.length;
      textarea.setSelectionRange(newCursorPosition, newCursorPosition);
    }, 0);
  };
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
        {/* Compact Header */}
        <div className="relative bg-gradient-to-r from-blue-600 via-blue-500 to-indigo-600 px-6 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2.5">
              ...
              <div>
                <h2 className="text-lg font-bold text-white">Приветственное сообщение</h2>
          ...
          <div className="p-5 space-y-4">
            ...
            <div className="inline-flex">
              <div 
                ...
                className="flex items-center space-x-3 p-3 bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl border border-blue-200 cursor-pointer hover:shadow-md transition-all"
              >
                <div className={`w-11 h-6 rounded-full transition-all duration-300 relative ${enabled ? 'bg-green-500' : 'bg-gray-300'}`}>
                  <div className={`absolute top-1 w-4 h-4 bg-white rounded-full shadow-md transition-all duration-300 ${enabled ? 'left-6' : 'left-1'}`}></div>
                </div>
                <div>
                  ...
                  <p className="text-xs text-gray-600">{enabled ? 'Новые участники будут получать сообщение' : 'Кликните для включения'}</p>
                </div>
                <input
                  type="checkbox"
                  id="enabled"
            ...
            {enabled && (
              <div className="space-y-4 animate-in slide-in-from-top duration-300">
                {/* Message Text Section - Optimized */}
                <div className="bg-gray-50 rounded-lg p-3.5 border border-gray-200">
                  <label className="flex items-center space-x-2 text-sm font-semibold text-gray-900 mb-2.5">
                  ...
                  <div className="flex flex-wrap gap-1 mb-2 p-2 bg-white rounded-lg border border-gray-200">
                    ...
                    <button
                      ...
                      className="p-1.5 hover:bg-gray-100 rounded transition-colors"
                      title="Зачеркнутый (strikethrough)"
                    >
                    ...
                    <button
                      ...
                      className="p-1.5 hover:bg-gray-100 rounded transition-colors"
                      title="Спойлер (spoiler)"
                    >
                      <span className="text-xs font-bold text-gray-700 px-1">||</span>
                    </button>
                    <button
                      type="button"
                  ...
                  <textarea
                    ...
                    rows={6}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-y transition-shadow shadow-sm text-sm min-h-[120px] max-h-[400px]"
                    placeholder="Привет, {MENTION}! 👋 Добро пожаловать в {GROUPNAME}!"
                  />
                  
                  {/* Variables Chips - Compact */}
                  <div className="mt-2.5 pt-2.5 border-t border-gray-300">
                    <p className="text-xs font-medium text-gray-700 mb-1.5">Переменные:</p>
                    <div className="flex flex-wrap gap-1.5">
                      {VARIABLES.map((variable) => (
                        <button
                          ...
                          className="group relative px-2 py-0.5 bg-white border border-blue-200 text-blue-700 text-xs font-medium rounded hover:bg-blue-600 hover:text-white hover:border-blue-600 transition-all shadow-sm"
                ...
                {/* Two Column Layout for Media and Lifetime */}
                  ...
                  <div className="bg-gray-50 rounded-lg p-3.5 border border-gray-200">
                    ...
                    {!mediaUrl ? (
                      <div className="border-2 border-dashed border-gray-300 rounded-lg p-3 hover:border-gray-400 transition-colors">
                        ...
                        <div className="text-center">
                          {uploadMediaMutation.isPending ? (
                            ...
                            <>
                              <Image className="mx-auto h-7 w-7 text-gray-400 mb-1.5" />
                              <label
                              ...
                              <p className="text-xs text-gray-500 mt-1">Фото или видео</p>
                            </>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="space-y-2">
                        <div className="flex items-center justify-between p-2.5 bg-white rounded-lg border-2 border-green-200 shadow-sm">
                          <div className="flex items-center space-x-2">
                            <div className={`p-1.5 rounded-lg ${mediaType === 'photo' ? 'bg-green-100' : 'bg-blue-100'}`}>
                              {mediaType === 'photo' ? <Image className="h-4 w-4 text-green-600" /> : <Video className="h-4 w-4 text-blue-600" />}
                            </div>
                            <div>
                  ...
                  <div className="bg-gray-50 rounded-lg p-3.5 border border-gray-200">
                    ...
                    <div className="flex items-center space-x-2">
                      <input
                        ...
                        min="1"
                        max="10080"
                        className="flex-1 px-2.5 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent shadow-sm text-sm"
                        placeholder="Не удалять"
                      />
                      <span className="text-sm text-gray-600 whitespace-nowrap">мин</span>
                    </div>
                    <p className="mt-2 text-xs text-gray-500 flex items-start">
                      <span className="mr-1">💡</span>
                      <span>Удалится через указанное время (макс. 7 дней)</span>
                    </p>
                  </div>
                </div>

                {/* Buttons Section - Optimized */}
                <div className="bg-gray-50 rounded-lg p-3.5 border border-gray-200">
                  <div className="flex items-center justify-between mb-2.5">
                    ...
                    <button
                      ...
                      className="flex items-center px-2.5 py-1.5 bg-indigo-600 text-white text-xs font-medium rounded-lg hover:bg-indigo-700 transition-colors shadow-sm"
                    >
                  ...
                  {buttons.length > 0 ? (
                    <div className="space-y-2">
                      {buttons.map((row, rowIndex) => (
                        <div key={rowIndex} className="bg-white border-2 border-indigo-100 rounded-lg p-2.5 shadow-sm">
                          <div className="flex items-center justify-between mb-2">
                            <span className="text-xs font-semibold text-indigo-600 px-1.5 py-0.5 bg-indigo-50 rounded">#{rowIndex + 1}</span>
                            <div className="flex items-center space-x-2">
                              <button
                                type="button"
                          ...
                          <div className="space-y-1.5">
                            {row.map((button, buttonIndex) => (
                              <div key={buttonIndex} className="flex items-center space-x-1.5">
                                <input
                                  type="text"
                                  value={button.text}
                                  onChange={(e) => updateButton(rowIndex, buttonIndex, 'text', e.target.value)}
                                  placeholder="Текст"
                                  className="flex-1 px-2 py-1.5 text-xs border border-gray-300 rounded-md focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                                />
                                <input
                                  type="url"
                                  value={button.url}
                    ...
                    <div className="text-center py-4 text-gray-400">
                      <Link2 className="h-6 w-6 mx-auto mb-1.5 opacity-30" />
        ...
        {/* Footer - Compact */}
        <div className="flex items-center justify-end space-x-3 px-5 py-3 border-t border-gray-200 bg-gray-50">
          <button
            type="button"
            onClick={onClose}
          ...
          <button
            ...
            className="px-5 py-2 bg-gradient-to-r from-blue-600 to-indigo-600 text-white text-sm font-semibold rounded-lg hover:from-blue-700 hover:to-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-md hover:shadow-lg"
          >
